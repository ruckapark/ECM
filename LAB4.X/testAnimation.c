...
//        ADResult += ((unsigned int)ADRESH << 8);            //get ADRresult at 10b num
        ...
        for (int i = 0; i < 50; i++){__delay_ms(10);}
...
//        }                                                   // Varying running speeds 
        ...
        char buf[16];                                       //space for display
        
        SendLCD(0b00000001,0);
        __delay_ms(3); //clear the display
        SendLCD(0b00000010,0);
        __delay_ms(3);//return home - useful for shifting left
        
        SetLine(1);                                         //set line 1
        
        SendLCD(57,1);
        //sprintf(buf,);
        //LCD_String(buf);                                 //send to the LCD
        ...
        characterCounter++;
        if(characterCounter == 3){characterCounter = 0;}    //reset to zero after three loops
