...
//    }
    ...
    int delayTime = 0;   // Varying delay time for speed
    
//    unsigned char leftHead = 0b00101000;
//    unsigned char rightHead = 0b00101001;
//    unsigned char backLeg = 0b00101101;
//    unsigned char straightLeg = 0b00101111;
//    unsigned char middleLegs = 0b00111011;
//    unsigned char frontLegs = 0b10110110;
//    unsigned char blankSpace = 0b00100000;
    
    unsigned char characterA_top[] = {0b00100000,0b00101000,0b00101001};
    unsigned char characterA_bottom[] = {0b00101101,0b00101111,0b00100000};
    unsigned char characterB_top[] = {0b00100000,0b00101000,0b00101001};
    unsigned char characterB_bottom[] = {0b00100000,0b00111011,0b00100000}; 
    unsigned char characterC_top[] = {0b00100000,0b00101000,0b00101001};
    unsigned char characterC_bottom[] = {0b00100000,0b00100000,0b10110110};
    
    char characterCounter = 0;
    char shiftCounter = 0;
    ...
    while(1){
        ...
        ADResult = ADRESL;
        ADResult += ((unsigned int)ADRESH << 8);            //get ADRresult at 10b num
        
        
        for (int i = 0; i < 10; i++){
        
            if (ADResult < 600) {__delay_ms(30);}
            else if (ADResult > 900) {__delay_ms(50);}
            else {__delay_ms(40);}
        }                                                   // Varying running speeds 
        
        
        SendLCD(0b00000001,0);
        __delay_ms(3); //clear the display
        
        SetLine(1);                                         //set line 1
        
        char counter = 0;
        while(counter < shiftCounter){
        SendLCD(0b00011100,0);
        counter++;
        }
        
        if (characterCounter == 0){
        for(int i = 0; i<3; i++){SendLCD(characterA_top[i],1);}
        } else if (characterCounter == 1){
        for(int i = 0; i<3; i++){SendLCD(characterB_top[i],1);}
        } else {
        for(int i = 0; i<3; i++){SendLCD(characterC_top[i],1);}
        }
                                         //send to the LCD
        
        SetLine(2); //set bottom line
        ...
        if (characterCounter == 0){
        for(int i = 0; i<3; i++){SendLCD(characterA_bottom[i],1);}
        } else if (characterCounter == 1){
        for(int i = 0; i<3; i++){SendLCD(characterB_bottom[i],1);}
        } else {
        for(int i = 0; i<3; i++){SendLCD(characterC_bottom[i],1);}
        }
        
        characterCounter++;
        if(characterCounter == 3){
            characterCounter = 0;           //reset to zero after three loops
            //SendLCD(0b00011100,0);
            shiftCounter++;
            if(shiftCounter == 6){shiftCounter = 0;}
        }    
