...
unsigned char stringToHex(unsigned char byte){
    if((byte >> 4) == 0b0011){
        byte = byte & 0b00001111;
    }else if(byte >> 4 == 0b0100){
        byte = 9 + (byte & 0b00001111);
    }
    
    return byte;
}

int mergeHex(unsigned char highByte, unsigned char lowByte){
    int byte;
    
    byte = (highByte << 4) + lowByte; //multiply high byte by 16
...
void main(void){
    ...
    int mergedRFIDdata[6];
    
    while(1){
        dataByte = getCharSerial();
        if(dataByte == 0x02){             // Check that the sequence sent starts with 0x02
            RFIDbuffer[bufIndex++] = getCharSerial();
            //bufIndex ++;
            while(RFIDbuffer[bufIndex - 1] != 0x03){      // Run this loop until the end byte 0x03 is read 
                //SendLCD(RCREG,1);      // Send the characters between 0x02 and 0x03 to LCD
                RFIDbuffer[bufIndex++] = getCharSerial();       // Wait until a new byte is received and check its not 0x03
                //bufIndex++;
            }
            bufIndex = 0;
        }
        ...
        while(bufIndex < 12){
            RFIDhex[bufIndex++] = stringToHex(RFIDbuffer[bufIndex]);
            //bufIndex++;
        }
        
        for(char i = 0; i<6; i++){
            mergedRFIDdata[i] = mergeHex(RFIDhex[i<<1], RFIDhex[(i<<1)+1]);
        }
        
        if ((mergedRFIDdata[0] ^ mergedRFIDdata[1] ^ mergedRFIDdata[2] ^ mergedRFIDdata[3] ^ mergedRFIDdata[4]) == mergedRFIDdata[5]){
            for(char i = 0; i < 10; i++){
               SendLCD(RFIDbuffer[i],1); 
            }
        }
